...
CommandNOTICE::CommandNOTICE(const DataStore& dataStore)
: CommandBase(), dataStore(dataStore) {
}

CommandNOTICE::CommandNOTICE(const CommandNOTICE& other)
...
responseList CommandNOTICE::execute(Client& client, const ParsedMessage& message) {
...
CommandBase* CommandNOTICE::clone() const {
    return new CommandNOTICE(*this);
}

responseList CommandNOTICE::handleClientRecipient(Client& sender, const std::string& recipientNick, const ParsedMessage& message) {
    responseList responses;
    ...
    singleResponse resp = createSingleResponse("NOTICE", recipient->getSocketFdString());
...
responseList CommandNOTICE::handleChannelRecipient(Client& sender, const std::string& channelName, const ParsedMessage& message) {
    responseList responses;
    ...
    singleResponse resp = createSingleResponse("NOTICE", memberFdStr);
    resp["<nick_sender>"] = sender.getNickname();
    resp["<user_sender>"] = sender.getUsername();
    resp["<host_sender>"] = sender.getHostname();
